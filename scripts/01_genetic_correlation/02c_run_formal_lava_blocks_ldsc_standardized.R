...
overlap_file <- file.path(in_dir, "lava_sample_overlap_ldsc_standardized.tsv")
...
mode_suffix <- if (use_overlap) "overlap_ldsc_standardized" else "no_overlap"
...
for (i in seq_len(nrow(loci))) {
  ...
  if (is.null(locus)) {
    append_row(qc_file, cbind(qc_base, status = "process_locus_null", message = "process.locus returned NULL"))
    next
  }
