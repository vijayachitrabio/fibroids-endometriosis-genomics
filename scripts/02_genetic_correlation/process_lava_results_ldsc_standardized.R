...
# Process the overlap-aware run whose sampling-correlation matrix was derived
# by standardising the full LDSC intercept covariance matrix.
results_file <- file.path(lava_dir, "LAVA_Formal_2495_Blocks_Results_overlap_ldsc_standardized_lava_loci_2495_blocks.csv")
...
out_s2 <- file.path(lava_dir, "Table_S2_LAVA_Local_rg_Formal_2495Blocks_overlap_ldsc_standardized_FDR05.csv")
fwrite(sig, out_s2)
...
out_full <- file.path(lava_dir, "Table_LAVA_Full_2495Blocks_AllResults_overlap_ldsc_standardized_withFDR.csv")
...
if (dir.exists(manu_dir)) {
  file.copy(out_s2, file.path(manu_dir, "Table_S2_LAVA_Local_rg_Formal_2495Blocks_overlap_ldsc_standardized_FDR05.csv"), overwrite = TRUE)
